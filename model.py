...
from keras.layers import Input, LSTM, Dense, Dropout,dot,concatenate,Activation
...
def seq2seq(hidden_size, nb_input_chars, nb_target_chars):
    ...
    attention = dot([decoder_outputs, decoder_enc_output], axes=[2, 2])
    attention = Activation('softmax')(attention)
    context = dot([attention, decoder_enc_output], axes=[2,1])
    decoder_outputs = concatenate([context, decoder_outputs])
    outputs = decoder_softmax(decoder_outputs)
    decoder_model = Model(inputs=[decoder_enc_output,decoder_inputs] + decoder_states_inputs,
                          outputs=[outputs] + decoder_states)
